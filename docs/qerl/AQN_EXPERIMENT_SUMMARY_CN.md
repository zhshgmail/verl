...
## 🔴 重要更正：鲁棒性测试结果错误 (2026-01-04)

### 之前的鲁棒性结果是错误的！

之前声称的"<1%退化"结果是**不正确的**，原因：
1. vLLM评估实际上**没有注入噪声**（多进程问题）
2. 早期的"原生PyTorch"结果从未用注入计数验证过

### ✅ 已验证的E5b结果（原生PyTorch，注入已确认）
...
- 模型**不具备**推理噪声鲁棒性：5%噪声下-14%，10%噪声下-28%
- 干净准确率(78%)略高于基线(76.88%)
- AQN有助于训练稳定性，但**不提供**推理鲁棒性

### ⚠️ 训练OOD指标存在误导

训练OOD准确率（如E5b: 70.58%）是在**5%噪声激活状态下**测量的：
- `noisy_ops`全局启用，包括验证阶段
- 将70.58%（带噪声）与基线76.88%（干净）比较是误导性的
- 真正的干净准确率约为78%
...
> 🔴 **重要更正 (2026-01-04)**:
>
> 以下旧结果是**错误的**！vLLM评估实际上没有注入噪声。
>
> **请参阅文档顶部的已验证E5b结果**（使用原生PyTorch，注入已确认）。
...
**🔴 更正后的结论**:
- 模型在5%推理噪声下退化**约14%**（不是<1%）
- 模型在10%推理噪声下退化**约28%**
- AQN**不提供**推理鲁棒性，只提供训练稳定性

> **⚠️ 更正说明：训练OOD vs 干净评估**
>
> 训练OOD准确率(70.58%)低于干净评估准确率(78%)的原因：
>
> | 测量 | 评估时噪声 | 结果 | 说明 |
> |------|-----------|------|------|
> | 训练最终准确率(OOD) | **有5%噪声** | 70.58% | noisy_ops全局开启，验证也带噪声 |
> | 干净评估@0% | **无噪声** | **78%** | 干净推理，展示模型真实能力 |
> | 噪声评估@5% | 有5%噪声 | **64%** | 与训练噪声相同，但退化14% |
> | 噪声评估@10% | 有10%噪声 | **50%** | 2倍训练噪声，退化28% |
>
> **关键洞察**：
> 1. 干净评估(78%)高于基线(76.88%)，说明噪声训练略微提升了干净性能
> 2. 但模型**没有**获得推理鲁棒性：5%噪声下从78%降至64%
> 3. AQN的作用是**训练稳定性**，不是推理鲁棒性
...
|------------|--------|--------|---------|---------|
| Step 232 | ~~89.50%~~ | ~~89.50%~~ | ~~89.00%~~ | ~~-0.50%~~ |

**✅ 新原生PyTorch结果（n=50，已验证）:**

| Checkpoint | 0% 噪声 | 5% 噪声 | 10% 噪声 | 前向注入次数 | 退化 |
|------------|--------|--------|---------|------------|------|
| Step 232 | **82%** (41/50) | **90%** (45/50) | **82%** (41/50) | 2.5M-2.7M | **0%** |

> ⚠️ **异常发现**: 7B模型在噪声下**没有退化**（5%噪声时准确率甚至上升）。
> 这与E5b（1.5B）形成对比，后者在5%噪声下退化14%。

**7B模型关键发现**:

1. **模型规模提升训练稳定性**: 7B噪声退化仅-1.97%（E7b vs E7a），远低于1.5B的-8.72%
2. **AQN仍然有效**: 即使7B本身更稳定，AQN仍提供+0.80%训练改进
3. **✅ 推理鲁棒性**: 7B模型对推理噪声表现出**强鲁棒性**（与1.5B不同）
4. **收益递减效应**: 7B的AQN改进（+0.80%）小于1.5B（+2.42%），符合预期

**7B vs 1.5B 鲁棒性对比**:

| 模型 | 0% 噪声 | 5% 噪声 | 10% 噪声 | 最大退化 |
|------|--------|--------|---------|---------|
| **E5b (1.5B)** | 78% | 64% | 50% | **-28%** |
| **E7c (7B)** | 82% | 90% | 82% | **0%** |

**结论**: 更大的模型对计算噪声具有更强的内在鲁棒性
