...
**✅ 新原生PyTorch结果（n=10，机制已验证）:**

| Checkpoint | 0% 噪声 | 5% 噪声 | 10% 噪声 | 前向注入次数 |
|------------|--------|--------|---------|------------|
| Step 232 | 70% (7/10) | 90% (9/10) | 90% (9/10) | 551K-569K |

> ⚠️ **说明**: 10个样本的统计意义不足，70%准确率在n=10时的方差约为±30%。
> 噪声注入机制**已确认正常工作**（551K+前向注入），但需要n≥100+样本才能得出有意义的结论。

**7B模型关键发现（训练相关，仍然有效）**:

1. **模型规模提升训练稳定性**: 7B噪声退化仅-1.97%（E7b vs E7a），远低于1.5B的-8.72%
2. **AQN仍然有效**: 即使7B本身更稳定，AQN仍提供+0.80%训练改进
3. ~~**极佳推理鲁棒性**~~: ❌ 此结论已作废，待重新验证
