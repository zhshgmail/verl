...
在生产环境中，硬件故障（GPU/NPU核心损坏、内存位翻转、算子实现差异等）会导致模型输出异常。**传统故障定位方法需要参考系统（已知正常的GPU）进行对比**，但存在以下困境：
...
| **GPU→NPU迁移** | 没有"正确答案"可对比 |

**具体痛点示例**：
- 模型从A100迁移到昇腾910B，准确率从89.5%降到71.2%
- 不知道问题出在哪一层、哪个算子
- 逐层对比需要同时运行两套硬件，成本高
- 二分法定位28层模型耗时长
...
1. **不依赖参考系统** - 单机即可诊断
2. **精确定位故障层** - 指出具体是哪一层有问题
3. **识别故障类型** - 噪声、死区、饱和、非确定性等

### 0.3 SRDD检测能力

| 故障类型 | 检测方法 | 准确率 |
|---------|---------|--------|
| Dead Zone（信号丢失） | 增益扫描 | 100% |
| Noise（随机噪声） | 不稳定性扫描 | 100% |
| Saturation（值截断） | 峰度扫描 | 100% (Dense) |
| 非确定性算子 | 不稳定性扫描 | 100% |
| Sparse故障 | 最小增益扫描 | 60% (3/5类型) |
...
> **向每一层注入已知扰动，观察响应是否符合预期。正常层增益≈1.0，故障层行为异常。**
