...
| 术语 | 英文 | 含义 |
|-----|------|------|
| **稀疏度** | Sparsity | 受故障影响的神经元比例。10%稀疏度 = 10%神经元故障，90%正常 |
| **Dense故障** | Dense Fault | 100%稀疏度，即该层所有神经元都受故障影响 |
| **Sparse故障** | Sparse Fault | <100%稀疏度，只有部分神经元受故障影响（模拟局部硬件损坏） |
| **增益** | Gain | 层的传递函数斜率。增益=输出变化/输入变化，健康层≈1.0 |
| **峰度** | Kurtosis | 分布"尖峰程度"的统计量。LLM激活值天然高峰度(≈3500) |
| **死区** | Dead Zone | 故障类型：小值被归零，导致信号丢失 |
| **饱和** | Saturation | 故障类型：值被截断到上限，导致尖峰被削平 |
| **边缘检测** | Edge Detection | 使用一阶导数找到指标"跳变"的位置，定位故障源 |
| **参考系统** | Reference System | 已知正常的GPU，用于对比检测故障 |
| **无参考** | Reference-free | 不需要已知正常的系统即可进行故障诊断 |

### 稀疏度示意图

```
100% 稀疏度 (Dense):     [X][X][X][X][X][X][X][X][X][X]  ← 所有神经元故障
                          故障 故障 故障 故障 故障 故障 故障 故障 故障 故障

10% 稀疏度 (Sparse):     [X][○][○][○][○][○][○][○][○][○]  ← 仅10%神经元故障
                          故障 正常 正常 正常 正常 正常 正常 正常 正常 正常

1% 稀疏度 (Very Sparse): [X][○][○][○][○]...(100个中只有1个故障)
```

**为什么稀疏故障难检测**: 当90%神经元正常时，全局统计量（如均值、峰度）被正常值"稀释"，难以观察到10%故障神经元的异常。
