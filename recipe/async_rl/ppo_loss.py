...
def decoupled_ppo_loss(
    ...
    """
    Compute decoupled PPO actor loss.
    ...
    Decoupled PPO:
        ...
        where:
        - r = π_θ / π_prox  (ratio against proximal policy)
        - w_behav = π_prox / π_old  (importance weight for behavior policy)

    Args:
        logprobs: Log probabilities from current policy π_θ. Shape: [bs, max_seqlen]
        proximal_logprobs: Log probabilities from proximal policy π_prox (recent policy
        ...
        behav_imp_weight_cap: Maximum allowed behavior importance weight (e.g., 10.0).
            Limits impact of very stale samples.
        use_decoupled_loss: If False, fall back to standard PPO (proximal_logprobs = old_logprobs)
    ...
    Implementation Notes:
        ...
        - proximal_logprobs = old_logprobs (both from same policy)
        - Behaves like standard PPO

        When decoupled loss is enabled:
        - proximal_logprobs: recomputed from recent checkpoint
        - old_logprobs: from inference engine (possibly stale)
        - Behavior importance weight: exp(proximal_logprobs - old_logprobs)
    """
    ...
    # Apply importance weight capping to limit impact of very stale samples
    if behav_imp_weight_cap is not None:
        behav_mask = (behav_imp_weight <= behav_imp_weight_cap).logical_and(loss_mask_bool)
    else:
        behav_mask = loss_mask_bool

    # Zero out importance weights for tokens that exceed the cap
