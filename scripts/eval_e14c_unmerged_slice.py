#!/usr/bin/env python3
"""
Single GPU slice evaluator for E14c - WITHOUT merging LoRA.
Called by eval_e14c_unmerged_parallel.sh.
"""
...
def main():
    ...
    print(f"[GPU {args.gpu_id}] Starting (UNMERGED LoRA)...")
    ...
    print(f"[GPU {args.gpu_id}] Loading LoRA (NOT merging)...")
    model = PeftModel.from_pretrained(base_model, LORA_ADAPTER)
    model.eval()

    # Apply NVFP4 (excluding LoRA modules)
    if args.nvfp4:
        print(f"[GPU {args.gpu_id}] Applying NVFP4 W4A4 (excluding LoRA)...")
