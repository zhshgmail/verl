#!/bin/bash
# MXFP4 v2.1: RMSNorm AQN (QeRL style) with lm_head fix
#
# Experiment ID: v2.1
# Date: 2026-01-09
#
...
# - RMSNorm AQN (QeRL default): sigma 0.05 -> 0.0005
# - 2 epochs
#
# This is the QeRL-style AQN approach with the lm_head fix.
# Compare with v2.0 (baseline, no AQN) to measure AQN benefit.
#
# Usage:
#   bash scripts/test_mxfp4_v2.1_rmsnorm_aqn.sh [N_GPUS]
...
OUTPUT_DIR="/tmp/mxfp4_v2.1_rmsnorm_aqn"
mkdir -p ${OUTPUT_DIR}

echo "=== MXFP4 v2.1: RMSNorm AQN (QeRL style), lm_head excluded, 2 epochs ==="
echo "Key settings:"
echo "  - MXFP4 W4A16 quantization"
echo "  - exclude_modules=['lm_head', 'embed_tokens'] (FIX!)"
echo "  - RMSNorm AQN: sigma 0.05 -> 0.0005 (10 stages)"
echo "  - 2 epochs"
echo "  - Compare with v2.0 (65.96%, no AQN)"
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.project_name=mxfp4_v2.1_rmsnorm_aqn \
    trainer.experiment_name=mxfp4_v2.1_rmsnorm_aqn_2ep \
    2>&1 | tee ${OUTPUT_DIR}/training.log

echo ""
echo "MXFP4 v2.1 experiment complete!"
echo "Results in: ${OUTPUT_DIR}"
echo ""
echo "Compare with:"
echo "  - v2.0 (baseline, no AQN): 65.96%"
echo "  - Old Exp 1E (RMSNorm AQN with lm_head bug): 62.32%"
