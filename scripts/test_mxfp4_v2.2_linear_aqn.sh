#!/bin/bash
# MXFP4 v2.2: Linear AQN with lm_head fix
#
# Experiment ID: v2.2
# Date: 2026-01-09
#
...
# - Linear AQN: sigma 0.001 -> 0.00001 (ultra-small, stability threshold)
# - 2 epochs
#
# Note: Previous experiments (1A, 1C) showed Linear AQN is unstable.
# sigma=0.001 is the stability threshold found in Exp 1D.
#
# Usage:
#   bash scripts/test_mxfp4_v2.2_linear_aqn.sh [N_GPUS]
...
OUTPUT_DIR="/tmp/mxfp4_v2.2_linear_aqn"
mkdir -p ${OUTPUT_DIR}

echo "=== MXFP4 v2.2: Linear AQN, lm_head excluded, 2 epochs ==="
echo "Key settings:"
echo "  - MXFP4 W4A16 quantization"
echo "  - exclude_modules=['lm_head', 'embed_tokens'] (FIX!)"
echo "  - Linear AQN: sigma 0.001 -> 0.00001 (stability threshold)"
echo "  - 2 epochs"
echo "  - Compare with v2.0 (65.96%, no AQN) and v2.1 (RMSNorm AQN)"
...
echo ""
echo "MXFP4 v2.2 experiment complete!"
echo "Results in: ${OUTPUT_DIR}"
echo ""
echo "Compare with:"
echo "  - v2.0 (baseline, no AQN): 65.96%"
echo "  - v2.1 (RMSNorm AQN): TBD"
echo "  - Old Exp 1D (Linear AQN with lm_head bug): 66.49%"
