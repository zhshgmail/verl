#!/bin/bash
# W4A4 MXFP4 + LoRA + Global RIN (E13i-baseline)
#
# Experiment ID: E13i-baseline
# Date: 2026-01-15
#
# Configuration:
# - W4A4 MXFP4 fake quantization (both weights AND activations)
# - 16-bit LoRA (rank=32, alpha=16)
# - Global RIN: sigma 0.05 -> 0.0005 on ALL layers (linear modules)
# - DAPO algorithm, 1 epoch
#
# Purpose: Test H1a (global RIN), H4a (aggressive decay), H5b (linear targets)
# Baseline: E13h = 71.42%
# Expected: 72-73% (+0.5 to +1.5% vs E13h)
#
# Usage:
#   bash scripts/test_mxfp4_w4a4_e13i_baseline.sh [N_GPUS]
...
OUTPUT_DIR="/tmp/mxfp4_w4a4_e13i_baseline"
mkdir -p ${OUTPUT_DIR}
...
echo "=== W4A4 MXFP4 + LoRA + Global RIN (E13i-baseline) ==="
echo "Key settings:"
echo "  - W4A4 MXFP4 fake quantization (injection_point=both)"
echo "  - 16-bit LoRA: rank=${lora_rank}, alpha=${lora_alpha}"
echo "  - Global RIN: sigma 0.05 -> 0.0005 on ALL layers (linear modules)"
echo "  - 10 decay stages (aggressive decay)"
echo "  - 1 epoch (29 steps)"
echo ""
echo "Baseline: E13h = 71.42%"
echo "Expected: 72-73% (+0.5 to +1.5%)"
...
python3 -m recipe.dapo.main_dapo \
    ...
    trainer.project_name=w4a4_e13i_baseline \
    trainer.experiment_name=mxfp4_w4a4_global_rin_1ep \
    trainer.logger='["console"]' \
    2>&1 | tee ${OUTPUT_DIR}/training.log

echo ""
echo "E13i-baseline W4A4 + Global RIN experiment complete!"
