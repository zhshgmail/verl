#!/bin/bash
# W4A4 MXFP4 + LoRA + Global RIN (E13i-v2 Lower Sigma)
#
# Experiment ID: E13i-v2
# Date: 2026-01-16
#
# Configuration:
# - W4A4 MXFP4 fake quantization (both weights AND activations)
# - 16-bit LoRA (rank=32, alpha=16)
# - Global RIN: sigma 0.01 -> 0.0001 on ALL layers (linear modules)
# - DAPO algorithm, 1 epoch
#
# Purpose: Test global RIN with W4A4-appropriate sigma (5x lower than W4A16)
# Background: E13i-baseline (σ=0.05) FAILED at step 3 - too aggressive for W4A4
# Baseline: E13h = 71.42%
# Expected: 71.8-72.2% (+0.4 to +0.8% vs E13h) - conservative due to lower sigma
#
# Usage:
#   bash scripts/test_mxfp4_w4a4_e13i_v2_lower_sigma.sh [N_GPUS]
...
OUTPUT_DIR="/tmp/mxfp4_w4a4_e13i_v2_lower_sigma"
mkdir -p ${OUTPUT_DIR}
...
echo "=== W4A4 MXFP4 + LoRA + Global RIN (E13i-v2 Lower Sigma) ==="
echo "Key settings:"
echo "  - W4A4 MXFP4 fake quantization (injection_point=both)"
echo "  - 16-bit LoRA: rank=${lora_rank}, alpha=${lora_alpha}"
echo "  - Global RIN: sigma 0.01 -> 0.0001 on ALL layers (linear modules)"
echo "  - 10 decay stages"
echo "  - 1 epoch (29 steps)"
echo ""
echo "Background: E13i-baseline (σ=0.05) FAILED - too aggressive for W4A4"
echo "Baseline: E13h = 71.42%"
echo "Expected: 71.8-72.2% (+0.4 to +0.8%)"
...
python3 -m recipe.dapo.main_dapo \
    ...
    trainer.project_name=w4a4_e13i_v2 \
    trainer.experiment_name=mxfp4_w4a4_global_rin_lower_sigma_1ep \
    trainer.logger='["console"]' \
    2>&1 | tee ${OUTPUT_DIR}/training.log

echo ""
echo "E13i-v2 W4A4 + Global RIN (Lower Sigma) experiment complete!"
