#!/bin/bash
# W4A4 MXFP4 + LoRA + Global AQN (E13j)
#
# Experiment ID: E13j
# Date: 2026-01-16
#
...
#
# Purpose: Validate bug fixes work, replicate QeRL's proven approach
# Fixes Applied:
#   1. filter_groups.enable=False (no rejection loop)
#   2. layer_types=["rmsnorm"] (match QeRL proven approach)
#
# Baseline: E13h = 71.42%
# Expected: Training completes (no filter rejection), score TBD
#
# Previous Failures:
#   - E13i-baseline: Failed step 3 (σ=0.05, Linear, filter=True)
#   - E13i-v2: Failed step 4 (σ=0.01, Linear, filter=True)
#
# Usage:
#   bash scripts/test_mxfp4_w4a4_e13j_global_aqn.sh [N_GPUS]
...
echo "=== W4A4 MXFP4 + LoRA + Global AQN (E13j) ==="
echo "Key settings:"
...
echo "Expected: Training completes without filter rejection"
...
python3 -m recipe.dapo.main_dapo \
    ...
    trainer.project_name=w4a4_e13j_global_aqn \
    trainer.experiment_name=mxfp4_w4a4_global_aqn_rmsnorm_1ep \
    trainer.logger='["console"]' \
    2>&1 | tee ${OUTPUT_DIR}/training.log

echo ""
echo "E13j W4A4 + Global AQN experiment complete!"
