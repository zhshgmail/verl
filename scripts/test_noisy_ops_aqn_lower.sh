#!/bin/bash
# Test Noisy Ops + Lower AQN (E5c) on A100
#
# This is E5c in the experimental series - testing LOWER AQN sigma
# to match QeRL paper values more closely.
#
# Previous results:
#   E5 (noise only): 68.16% accuracy
#   E5a (noise + global AQN σ=0.05→0.0005): 68.76% (+0.60%)
#   E5b (noise + epoch-aware AQN σ=0.05→0.0005): 70.58% (+2.42%)
#
# E5c hypothesis:
#   Lower AQN noise (σ=0.01→0.00001) may improve convergence
#   while maintaining robustness benefit.
#
# AQN schedule (Lower values):
#   Epoch 1: sigma 0.01 → 0.002 (5x lower than E5b)
#   Epoch 2: sigma 0.002 → 0.00001 (refinement)
#
# Usage: bash scripts/test_noisy_ops_aqn_lower.sh [ERROR_SCALE] [N_GPUS]
#
...
# This simulates 5% relative error in all matmul operations
...
COMMON_ARGS="
    ...
    trainer.nnodes=1
    trainer.project_name=noisy_ops_aqn_lower_test
    trainer.n_gpus_per_node=${N_GPUS}
    trainer.val_before_train=True
"

echo "=== Running Noisy Ops + Lower AQN Test (E5c) ==="
...
echo "  2. LOWER Epoch-aware AQN (E5c):"
echo "     - sigma_start: 0.01 (5x lower than E5b's 0.05)"
echo "     - sigma_end: 0.00001 (50x lower than E5b's 0.0005)"
echo ""
echo "Comparison with previous experiments:"
echo "  E5 (noise only): 68.16%"
echo "  E5a (noise + global AQN σ=0.05→0.0005): 68.76%"
echo "  E5b (noise + epoch-aware AQN σ=0.05→0.0005): 70.58%"
echo "  E5c (noise + epoch-aware AQN σ=0.01→0.00001): TBD"
echo ""
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name=noisy_ops_aqn_lower_${ERROR_SCALE}

echo "=== E5c Test Complete ==="
echo "Compare result with:"
echo "  E5b (σ=0.05→0.0005): 70.58%"
