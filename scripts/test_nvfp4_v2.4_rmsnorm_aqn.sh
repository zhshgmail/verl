#!/bin/bash
# NVFP4 v2.4: RMSNorm AQN (QeRL style) with lm_head fix
#
# Experiment ID: v2.4
# Date: 2026-01-09
#
...
# - NVFP4 has lower error rate (~1% vs ~21% for MXFP4)
# - AQN should be more effective with lower quantization error
#
# Usage:
#   bash scripts/test_nvfp4_v2.4_rmsnorm_aqn.sh [N_GPUS]
...
OUTPUT_DIR="/tmp/nvfp4_v2.4_rmsnorm_aqn"
mkdir -p ${OUTPUT_DIR}

echo "=== NVFP4 v2.4: RMSNorm AQN (QeRL style), lm_head excluded, 2 epochs ==="
echo "Key settings:"
...
echo "  - 2 epochs"
echo "  - Compare with v2.3 (NVFP4 baseline)"
echo ""
echo "This is expected to be the BEST performing configuration!"
...
echo ""
echo "NVFP4 v2.4 experiment complete!"
echo "Results in: ${OUTPUT_DIR}"
echo ""
echo "Compare with:"
echo "  - NVFP4 v2.3 (baseline, no AQN): TBD"
echo "  - MXFP4 v2.0 (baseline): 65.96%"
echo "  - MXFP4 v2.1 (RMSNorm AQN): TBD"
