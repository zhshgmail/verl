#!/bin/bash
# E13e: NVFP4 W4A4 with base_layer excluded
#
# BUG FOUND: We were quantizing BOTH q_proj AND q_proj.base_layer
# The base_layer is the underlying nn.Linear inside LoRA wrapper
# This causes duplicate/conflicting quantization
#
# FIX: Add 'base_layer' to exclusion list
#
# Previous E13d (PRE-hook): 8.49% at step 0 - FAILED
# This test: Same as E13d but with base_layer excluded

cd /home/z00637938/workspace/verl

# Configuration
OUTPUT_DIR="/tmp/nvfp4_w4a4_exclude_baselayer_e13e"
LOG_FILE="${OUTPUT_DIR}/training.log"
mkdir -p "${OUTPUT_DIR}"

echo "E13e-exclude-baselayer started in $(hostname) (PID: $$)" > /tmp/nvfp4_w4a4_e13e_status.txt

# Run training with base_layer excluded
nohup env WANDB_MODE=disabled python3 -m recipe.dapo.main_dapo \
    ...
    trainer.project_name=w4a4_e13e_exclude_baselayer \
    trainer.experiment_name=nvfp4_w4a4_exclude_baselayer_1ep \
    > "${LOG_FILE}" 2>&1 &

echo "NVFP4 W4A4 with base_layer excluded started (PID: $!)"
echo "KEY CHANGE: Added 'base_layer' to exclusion list"
