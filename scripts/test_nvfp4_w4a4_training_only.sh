#!/bin/bash
# E13c: NVFP4 W4A4 with apply_during=training (NOT during vLLM rollout)
#
# HYPOTHESIS: QeRL might only apply quantization during training forward pass,
# not during vLLM inference/rollout generation.
#
# Key change: apply_during=training instead of apply_during=both
#
# If this works better, it means vLLM rollout with W4A4 quantization is the problem.

cd /home/z00637938/workspace/verl

# Configuration
OUTPUT_DIR="/tmp/nvfp4_w4a4_training_only_e13c"
LOG_FILE="${OUTPUT_DIR}/training.log"
mkdir -p "${OUTPUT_DIR}"

echo "E13c-nvfp4-training-only started in $(hostname) (PID: $$)" > /tmp/nvfp4_w4a4_e13c_status.txt

# Run training with NVFP4 W4A4 - TRAINING ONLY (not during vLLM rollout)
nohup env WANDB_MODE=disabled python3 -m recipe.dapo.main_dapo \
    ...
    trainer.hw_error_injection.apply_during=training \
    ...
    trainer.project_name=w4a4_e13c_training_only \
    trainer.experiment_name=nvfp4_w4a4_training_only_1ep \
    > "${LOG_FILE}" 2>&1 &

echo "NVFP4 W4A4 (training-only) started (PID: $!)"
echo "Key change: apply_during=training (NOT both)"
